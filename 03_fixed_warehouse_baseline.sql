/*
================================================================================
  JPMC MERCHANT SERVICES - FIXED WAREHOUSE (LARGE) BASELINE
  ================================================================================

  PURPOSE: Run the same 8 queries on a FIXED Large warehouse to compare
  performance and cost against the Adaptive Warehouse.

  This script demonstrates the "BEFORE" state — where the team uses a single
  fixed-size warehouse (Large) for ALL workloads:
  - Simple lookups: OVER-PROVISIONED (paying for L when XS would suffice)
  - Complex analytics: UNDER-PROVISIONED (L is too small, queries are slow)

  Compare query execution times from this script vs 02_adaptive_warehouse_demo.sql
  to show the customer the difference.

  WAREHOUSE: JPMC_MERCHANT_L_WH (Large = 8 credits/hour)
  ================================================================================
*/
...
-- On Large WH: OVER-PROVISIONED — L is overkill for a simple GROUP BY
-- =============================================================================
...
-- =============================================================================
-- QUERY 3: MEDIUM SCAN WITH FILTER
-- On Large WH: Slightly over-provisioned but closer to appropriate
-- =============================================================================
...
-- On Large WH: Borderline — may be adequate or slightly under-provisioned
-- =============================================================================
...
-- =============================================================================
-- QUERY 5: COMPLEX WINDOW FUNCTIONS
-- On Large WH: UNDER-PROVISIONED — window functions over large data need more
-- =============================================================================
...
-- =============================================================================
-- QUERY 6: HEAVY FULL-TABLE ANALYTICS
-- On Large WH: UNDER-PROVISIONED — full table scan with joins needs XL
-- =============================================================================
...
-- =============================================================================
-- QUERY 7: COMPLEX MULTI-JOIN WITH SUBQUERIES
-- On Large WH: UNDER-PROVISIONED — complex CTEs over 500M rows need XL
-- =============================================================================
...
-- =============================================================================
-- QUERY 8: MASSIVE CROSS-TABLE AGGREGATION
-- On Large WH: SIGNIFICANTLY UNDER-PROVISIONED — needs XL+ for this workload
-- =============================================================================
...
/*
  ...
  FIXED LARGE WAREHOUSE:
  - Simple queries (Q1, Q2): Complete fast BUT you're paying 8 credits/hr
    regardless — wasteful for sub-second queries
  - Complex queries (Q6, Q7, Q8): SLOWER than adaptive because L doesn't
    have enough compute for 500M row scans with joins
  ...
  "With a fixed Large warehouse, you're either over-paying for simple work
   or under-performing on complex work. Adaptive eliminates both problems
   by right-sizing every single query automatically."
  ================================================================================
*/
