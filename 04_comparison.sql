...
SELECT
    ...
    DATEDIFF('SECOND', $FIXED_START, $FIXED_END) + 60 AS TOTAL_UPTIME_SEC,
    8 AS CREDITS_PER_HOUR,
    ROUND((DATEDIFF('SECOND', $FIXED_START, $FIXED_END) + 60) / 3600.0 * 8, 4) AS ESTIMATED_CREDITS
...
WITH fixed_cost AS (
    SELECT
        ROUND((DATEDIFF('SECOND', $FIXED_START, $FIXED_END) + 60) / 3600.0 * 8, 4) AS CREDITS
),
adaptive_cost AS (
...
SELECT
    ...
    DATEDIFF('SECOND', $FIXED_START, $FIXED_END) + 60 AS UPTIME_SEC,
