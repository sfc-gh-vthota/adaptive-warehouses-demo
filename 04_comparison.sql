...
FROM TABLE(INFORMATION_SCHEMA.WAREHOUSE_METERING_HISTORY(
    DATE_RANGE_START => LEAST($ADAPTIVE_START, $FIXED_START),
    DATE_RANGE_END => DATEADD(MINUTE, 10, GREATEST($ADAPTIVE_END, $FIXED_END))
))
...
adaptive_cost AS (
    ...
    FROM TABLE(INFORMATION_SCHEMA.WAREHOUSE_METERING_HISTORY(
        ...
        DATE_RANGE_END => DATEADD(MINUTE, 10, $ADAPTIVE_END)
    ))
